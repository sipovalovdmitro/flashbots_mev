    ...
    object "runtime" {
        code {
            ...
            case 0xe3834899 {
                ...
                mstore(add(ptr, 0x20), pair)
                mstore(add(ptr, 0x40), amountIn)
                let success := call(gas(), tokenIn, 0, add(ptr, 0x1c), 0x44, add(ptr, 0x60), 0x20)
                let result := mload(add(ptr, 0x60))
                if or(iszero(success), iszero(result)) {
                    revert(0, 0)
                }
            ...
            case 0x7af1c926 {
                ...
                mstore(add(ptr, 0x20), pair)
                mstore(add(ptr, 0x40), amountIn)
                let success := call(gas(), weth, 0, add(ptr, 0x1c), 0x44, add(ptr, 0x60), 0x20)
                let result := mload(add(ptr, 0x60))
                if or(iszero(success), iszero(result)) {
                    revert(0, 0)
                }
